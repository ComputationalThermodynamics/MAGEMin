...
if (gv.output_matlab == 1){
		...
		fprintf(loc_min, "%6s%15s %13s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s\n","phase","fraction[wt]","G[J]" ,"Vm[cm3/mol]","Vpart[cm3]" ,"Cp[kJ/K]","Rho[kg/m3]","Alpha[1/K]","Entropy[J/K]","Enthalpy[J]","BulkMod[GPa]","ShearMod[GPa]","Vp[km/s]","Vs[km/s]");
		...
		for (int i = 0; i < gv.len_cp; i++){
			if (cp[i].ss_flags[1] == 1){
				...
				fprintf(loc_min, "%+15.5f %+13.5f %+12.5f %+12.5f %+12.5f %+12.5f %+12.8f %+12.6f %+14.4f %+12.2f %+12.2f %+12.2f %+12.2f",
							sp[0].ph_frac_wt[n],
							G,
							cp[i].volume*10.,
							cp[i].volume*10.*cp[i].ss_n_mol,
							cp[i].phase_cp,
							cp[i].phase_density,
		...
		for (int i = 0; i < gv.len_pp; i++){
			if (gv.pp_flags[i][1] == 1){ 
				...
				fprintf(loc_min, "%+15.5f %+13.5f %+12.5f %+12.5f %+12.5f %+12.5f %+12.8f %+12.6f %+14.4f %+12.2f %+12.2f %+12.2f %+12.2f",
						...
						PP_ref_db[i].volume*10.*gv.pp_n[i],
		...
		fprintf(loc_min, "%6s %14s %+13.5f %12s %+12.5f %12s %+12.5f %12s %+12.6f %+14.4f %+12.5f %+12.5f %+12.5f %+12.5f\n",
				"SYS",
				" ",
				G,
				" ",
